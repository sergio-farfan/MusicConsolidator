...
public func validateMergePlanIntegrity(_ plan: MergePlan) throws {
    ...
    // targetDescription/sourceNames/targetName too (freeFormMergeFingerprint),
    // so a hand-edited description, source-name list, or target name —
    // recomputed from the SAME persisted fields checked above — no longer
    // agrees with the persisted fingerprint and fails closed here, even
    // though the canonical-recompute-and-diff below cannot see that tamper
    // on its own (it necessarily echoes the persisted fields back as its
    // own input). Same-name plans are UNCHANGED: `mergeFingerprint(_:)`
    ...
    if expectedFingerprint != plan.mergeFingerprint {
        // Final review deferred cosmetic: a free-form mismatch is caught
        // over PERSISTED PLAN FIELDS (copies, description, source names,
        // target name), not just "copies" — the same-name message stays
        // exactly as it always was.
        throw PlanIntegrityError(
            plan.isFreeForm
                ? "merge fingerprint does not match the persisted plan fields"
                : "merge fingerprint does not match the persisted copies"
        )
    }
